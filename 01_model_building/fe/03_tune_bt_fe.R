...
final_wflow <- tune_bt_fe |> 
  ...
  finalize_workflow(select_best(tune_bt_fe, metric = "roc_auc"))

# train final model 
set.seed(1234)
final_tune_bt_fe <- fit(final_wflow, train)

# compare to test submission  ----
load(here("data/test.rda"))
submission_test_bt_fe <- bind_cols(test, predict(final_tune_bt_fe, test, type = "prob")) |> 
  mutate(predicted = .pred_TRUE) |> 
  select(id, predicted)

# save submission test ----
write_csv(submission_test_bt_fe, file = here("submissions/submission_test_bt_fe.csv"))