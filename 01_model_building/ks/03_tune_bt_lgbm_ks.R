...
# Define and tune lgbm bt model on ks recipe ----
...
# show_best(tune_lgbm_bt_ks, metric = "roc_auc")

# collect metrics ----
# extract final workflow 
final_wflow <- tune_lgbm_bt_ks |> 
  extract_workflow(tune_lgbm_bt_ks) |>  
  finalize_workflow(select_best(tune_lgbm_bt_ks, metric = "roc_auc"))

# train final model 
set.seed(1234)
final_tune_bt_lgbm_ks <- fit(final_wflow, train)

# compare to test submission  ----
load(here("data/test.rda"))
submission_test_bt_lgbm_ks <- bind_cols(test, predict(final_tune_bt_lgbm_ks, test, type = "prob")) |> 
  mutate(predicted = .pred_TRUE) |> 
  select(id, predicted)

# save submission test ----
write_csv(submission_test_bt_lgbm_ks, file = here("submissions/submission_test_bt_lgbm_ks.csv"))

# roc_auc is 0.96238
# BT LGBM: KS: 291.09 sec elapsed
