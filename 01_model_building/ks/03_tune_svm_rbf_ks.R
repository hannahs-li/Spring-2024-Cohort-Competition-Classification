...
final_wflow <- tune_svm_rbf_ks |> 
  ...
  finalize_workflow(select_best(tune_svm_rbf_ks, metric = "roc_auc"))

# train final model 
set.seed(1234)
final_tune_svm_rbf_ks <- fit(final_wflow, train)

# compare to test submission  ----
submission_test_svm_rbf_ks <- bind_cols(test, predict(final_tune_svm_rbf_ks, test, type = "prob")) |> 
  mutate(predicted = .pred_TRUE) |> 
  select(id, predicted)

# save submission test
write_csv(submission_test_svm_rbf_ks, file = here("submissions/submission_test_svm_rbf_ks.csv"))