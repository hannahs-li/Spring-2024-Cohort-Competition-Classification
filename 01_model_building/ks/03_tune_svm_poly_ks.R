...
final_wflow <- tune_svm_poly_ks |> 
  ...
  finalize_workflow(select_best(tune_svm_poly_ks, metric = "roc_auc"))

# train final model 
set.seed(1234)
final_tune_svm_poly_ks <- fit(final_wflow, train)

submission_test_svm_poly_ks <- bind_cols(test, predict(final_tune_svm_poly_ks, test, type = "prob")) |> 
  mutate(predicted = .pred_TRUE) |> 
  select(id, predicted)

# save submission test ----
write_csv(submission_test_svm_poly_ks, file = here("submissions/submission_test_svm_poly_ks.csv"))
